// One-stop header.
#include </home/lola/libtorch-shared-with-deps-latest/libtorch/include/torch/script.h>
...
bool LoadImage(std::string file_name, cv::Mat &image) {
  image = cv::imread(file_name, 1);  // CV_8UC3
...
int main(int argc, const char *argv[]) {
  if (argc != 3) {
    std::cerr << "Usage: LibTorch_inference </home/lola/LibTorch_inference/Mnist_LeNet.pt> "
                 "</home/lola/LibTorch_inference/labels.txt>"
              << std::endl;
    return -1;
  }
  ...
  torch::jit::script::Module module;
     try {
     // Deserialization: Import TorchScript model
